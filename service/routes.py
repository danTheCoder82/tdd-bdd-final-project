...
def list_products():
    ...
    app.logger.info("Number of products: %d", len(results))
...
def get_products(product_id):
    ...
    if product is None:
        abort(status.HTTP_404_NOT_FOUND, f"No product with id {product_id} exists.")
...
def update_products(product_id):
    ...
    if product is None:
        abort(status.HTTP_404_NOT_FOUND, f"No product found for id {product_id}.")
